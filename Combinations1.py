...
def read_input_RS ():
    lower_bound = (np.transpose(np.loadtxt('LowerArray.csv',
                  delimiter=',', skiprows=1)))
    upper_bound = (np.transpose(np.loadtxt('UpperArray.csv',
                delimiter=',', skiprows=1)))
...
def read_probability_assignment ():
    lower_assignment = (np.transpose(np.loadtxt(
        'LowerProbabilityAssignment.csv', delimiter=',', skiprows=1)))
    upper_assignment = (np.transpose(np.loadtxt(
        'UpperProbabilityAssignment.csv', delimiter=',', skiprows=1)))
    return lower_assignment, upper_assignment

def independent_probabilty (probability_assigment: object) -> object:
...
if __name__ == "__main__":
    ...
    FOS1,low,FOS2, upper= (random_sets(FOS1,independent_lower, FOS2,
                                       independent_upper ))
